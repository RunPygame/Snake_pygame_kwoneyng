...
while True:
    ...
    for event in events:
        ...
        if event.type == MOUSEBUTTONDOWN:
            ...
            bd.shots.stats.append([bd.shots.dmg, math.atan2(position[1]+bd.player.size//2-bd.player.position[0], position[0]+bd.player.size//2-bd.player.position[1]), bd.player.position[0], bd.player.position[1]])
        ...
        for enm in bd.enms.stats:
            ...
            if hit == 0:
                if collision(y1,x1,size1,y2,x2,size2):
                    ...
                    hit = 1
        if hit == 0:
            if 0 <= y1+math.cos(pal)*bd.shots.spd < bd.height and 200 <= x1+math.sin(pal)*bd.shots.spd < bd.width:      
                bd.shots.stats.append([dmg, pal, y1+math.sin(pal)*bd.shots.spd, x1+math.cos(pal)*bd.shots.spd]) # 삼각함수 들어가야됨
